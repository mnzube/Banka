  ...
  static signup(req, res) {
    ...
    User.create(newUser)
      .then((user) => {
        ...
        if (save) {
          const payload = { id: save.id };
          jwt.sign(payload, process.env.secret, { expiresIn: "24d" }, (err, token) => {
      ...
      .then((users) => {
        if (users.rows.length !== 0) {
          ...
          bcrypt.compare(req.body.password, user.password, (error, matches) => {
            if (error) { return res.status(500).json({ status: 500, error: "authentication error" }); }
            if (!matches) { return res.status(400).json({ status: 400, error: "passwords don't match." }); }
            const payload = { id: user.id }; jwt.sign(payload, process.env.secret, { expiresIn: "24d" },
              (err, token) => {
                if (err) { console.log(err); }
                return res.status(200).json({ status: 200, token: `${token}`, data: { message: "User sucessfully signed in" } });
              });
          });
        } else { return res.status(400).json({ status: 400, error: "authentication failed." }); }
      })
      .catch((error) => {
        console.log(error);
      });
